...
# -*- coding: utf-8 -*-
"""
台股日K策略選股：掃描 tickers.json 股票標的，將訊號寫入 TW_Strategy.json。
...
OUTPUT_JSON = ROOT / "TW_Strategy.json"
TICKERS_JSON = ROOT / "tickers.json"
UNIVERSE_NAME = "tickers.json 台股"
YF_SUFFIXES = (".TW", ".TWO")
...
def load_universe() -> dict[str, str]:
    """
    從 tickers.json 載入台股普通股（略過股票期貨），依代碼去重。

    @returns {四位數代碼: 公司名稱}
    """
    if not TICKERS_JSON.exists():
        raise FileNotFoundError(f"找不到標的清單：{TICKERS_JSON}")
    try:
        data = json.loads(TICKERS_JSON.read_text(encoding="utf-8"))
    except (OSError, json.JSONDecodeError) as exc:
        raise RuntimeError(f"無法解析 {TICKERS_JSON.name}：{exc}") from exc

    mapping: dict[str, str] = {}
    skipped_groups = 0
    for group in data.get("groups", []) if isinstance(data, dict) else []:
        if not isinstance(group, dict) or group.get("market") != "stock":
            skipped_groups += 1
            continue
    ...
    if not mapping:
        raise RuntimeError(f"{TICKERS_JSON.name} 沒有可用的 stock 標的")
    log(f"已載入 {TICKERS_JSON.name} 台股 {len(mapping)} 檔（略過非股票族群 {skipped_groups} 組）")
    return mapping
...
def download_yf_symbols(yf_tickers: list[str]) -> dict[str, pd.DataFrame]:
    """
    分批下載 yfinance 代碼的日K，失敗的批次會改為逐檔重試。

    @param yf_tickers yfinance 代碼（例如 2330.TW）
    @returns {yfinance 代碼: 日K DataFrame}
    """
    history: dict[str, pd.DataFrame] = {}
    batches = chunked(yf_tickers, BATCH_SIZE)
    for index, batch in enumerate(batches, start=1):
        ...
        if missing and raw is None:
            log(f"批次下載失敗，缺 {len(missing)} 檔，改為逐檔下載")
            ...
            log(f"批次缺 {len(missing)} 檔（可能非此交易所），稍後改試其他後綴")
        time.sleep(0.4)
    return history


def download_history(codes: list[str]) -> dict[str, pd.DataFrame]:
    """
    下載台股日K：先試上市 `.TW`，沒資料再試上櫃 `.TWO`。

    @param codes 四位數股票代碼
    @returns {四位數代碼: 日K DataFrame}
    """
    history: dict[str, pd.DataFrame] = {}
    pending = list(codes)
    for suffix in YF_SUFFIXES:
        if not pending:
            break
        label = "上市 .TW" if suffix == ".TW" else "上櫃 .TWO"
        log(f"嘗試 {label}：{len(pending)} 檔")
        yf_list = [f"{code}{suffix}" for code in pending]
        yf_to_code = dict(zip(yf_list, pending, strict=True))
        batch_hist = download_yf_symbols(yf_list)
        found = 0
        for yf_symbol, df in batch_hist.items():
            code = yf_to_code.get(yf_symbol)
            if not code:
                continue
            history[code] = df
            found += 1
        pending = [code for code in pending if code not in history]
        log(f"{label} 成功 {found} 檔，剩餘 {len(pending)} 檔")
    return history
...
def main() -> int:
    ...
    payload = {
        "meta": {
            ...
            "note": "日期與時間為台灣時區；signal_date 為最新日K的台股交易日。僅保留近 10 日（含當日）選股，超過 10 日前的紀錄會刪除。掃描宇宙為 tickers.json 的 stock 族群（去重）。",
        },
        "picks": kept,
    }
    write_json(payload)
