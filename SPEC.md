# TW Strategy View 規格說明

本專案用 Python + `yfinance` 掃描 **tickers.json 台股普通股** 日K，依 S1 / S2 / S3 / S4 產出選股（須先通過最新K線收盤價 `> 30` 且成交量 `> 2M`），寫入 `TW_Strategy.json`（每日新增、不整檔覆蓋，累積近 10 日並刪除更舊資料），再以 `index.html` 深色頁面顯示（**預設只看最新一日**，可用日期範圍查舊資料）。GitHub Actions 於**台灣時間每天 15:00**（台股收盤後）自動執行，並可部署到 GitHub Pages。

策略條件與 [US_Strategy_View](https://github.com/Johsok/US_Strategy_View) 相同；僅掃描宇宙改為台股標的。
...
| 檔案 | 用途 |
| --- | --- |
| `TW_Strategy.py` | 選股主程式，將當日結果新增至 JSON，並刪除超過 10 日前的資料 |
| `TW_Strategy.json` | 近 10 日選股結果（每日新增、不整檔覆蓋；超過 10 日前的資料會刪除） |
| `tickers.json` | 台股掃描宇宙（只取 `market: stock` 族群，代碼去重；略過股票期貨） |
| `index.html` | 深色模式檢視頁；預設只顯示最新一日，可用日期範圍查舊資料 |
| `requirements.txt` | Python 套件 |
| `.github/workflows/tw-strategy.yml` | GitHub 每日排程 |
...
| `symbol` | 台股四位數代碼（例如 `2330`） |
| `strategy_desc` | 策略說明 |
| `strategy` | `S1` / `S2` / `S3` / `S4` |
| `side` | `buy`（買進）或 `short`（賣空） |
| `signal_date` | 最新日K的台股交易日 |
...
每次執行 `TW_Strategy.py` 時：

1. 讀取既有 `TW_Strategy.json` 的 `picks`（檔案不存在或損毀則視為空白）。
...
例：執行日為 `2026-08-23` 時，保留 `2026-08-13`（含）之後的選股，刪除 `2026-08-12` 及更早的紀錄。同一日重複執行只更新當日，不會覆蓋其他日期。

### 2.2 網頁顯示（最新一日 + 日期範圍）

`index.html` 讀取整個 `TW_Strategy.json`，但畫面規則如下：
...
1. **不顯示頁尾統計說明**：頁面最下方**不放**「掃描宇宙…成功 x / x…」這類說明列，之後也不再加回。
2. **表格不用內捲軸**：表格完整展開列出全部資料，不設 `max-height`、不用容器內捲軸，捲動一律交給**頁面外捲軸**；表頭捲動時吸附在視窗頂端（sticky）。
3. **頂部漸層捲動進度條**：頁面最上方固定一條 4px 進度條，背景為 `linear-gradient(90deg, #00f260, #0575e6)` 漸層並帶光暈，以 `scroll` 事件搭配 `requestAnimationFrame` 依頁面捲動比例更新寬度。
...
7. 點代碼開啟 [Yahoo 股市](https://tw.stock.yahoo.com/) 個股頁。

---

## 3. 選股規則（日K）

K線顏色採**與美股版相同慣例：綠漲、紅跌**（收盤 > 開盤為綠K，收盤 < 開盤為紅K）。漲跌幅為相對**前一日收盤**。
...
掃描宇宙為 `tickers.json` 的 `stock` 族群（去重）。下載日K時先試 Yahoo 上市代碼 `{代號}.TW`，沒資料再試上櫃 `{代號}.TWO`。
...
目標：每天台灣 15:00 跑 `TW_Strategy.py` → 更新 `TW_Strategy.json` → 用 GitHub Pages 公開 `index.html`。

### 5.1 建立 GitHub 儲存庫並推檔

1. 登入 [GitHub](https://github.com)，右上角 **New repository**。
2. Repository name 例如 `TW_Strategy_View`。可選 Public 或 Private（Private 的 GitHub Pages 需要付費方案）。
...
cd TW_Strategy_View
git init
git add TW_Strategy.py TW_Strategy.json tickers.json index.html requirements.txt SPEC.md .gitignore .nojekyll .github
git commit -m "feat: 台股日K策略選股與每日排程"
git branch -M main
git remote add origin https://github.com/你的帳號/TW_Strategy_View.git
...
`https://你的帳號.github.io/TW_Strategy_View/`

### 5.4 第一次請「手動」跑一次（不要只等隔天）

排程最早要等到下一個台灣 15:00。請先手動確認：

1. repo 上方 **Actions**。
2. 左側選 **Daily TW Strategy**。
3. 右側 **Run workflow** → 分支選 `main` → **Run workflow**。
4. 點進該次 run，確認兩個 job：
   - `scan`：安裝套件、執行 `TW_Strategy.py`、commit `TW_Strategy.json`
   - `pages`：把 `index.html` + `TW_Strategy.json` 佈到 Pages
5. 兩個都是綠色後，打開 Pages 網址，應看到台灣日期／時間與選股卡片。

`scan` 可能要數分鐘（下載約 300 檔日K）。Yahoo 偶發擋 IP 時，程式會分批重試；部分代碼失敗會寫在 JSON 的 `meta.failed`。

### 5.5 每日 15:00 如何對應 GitHub cron

GitHub cron 只用 **UTC**。

| 台灣時間 | UTC | yml |
| --- | --- | --- |
| 每天 15:00 | 07:00 | `cron: "0 7 * * *"` |

台股約 13:30 收盤，15:00 通常已有完整日K。若 JSON 常缺當日K，把 cron 改成 `0 8 * * *`（台灣 16:00）即可。

可在 [crontab.guru](https://crontab.guru/) 檢查。

### 5.6 工作流程實際做什麼

`.github/workflows/tw-strategy.yml` 會：

1. 被 **排程** 或 **手動 Run workflow** 觸發（不會因為你一般 push 程式碼而重複掃股）。
2. 用 Python 3.12 安裝 `requirements.txt`。
3. 執行 `python TW_Strategy.py`。
4. 若 JSON 有變化，以 `github-actions[bot]` 提交並 push 回 `main`。
5. 上傳 `index.html` + `TW_Strategy.json` 到 GitHub Pages。

HTML 用相對路徑讀 `TW_Strategy.json`，與 Python「執行」是分開的：Python 負責產出資料，Pages 只負責靜態顯示。
...
| 網頁空白或 fetch 失敗 | 確認 Pages 網址路徑正確，且該次部署有包含 `TW_Strategy.json` |
...
| 想改掃描清單 | 編輯 `tickers.json` 的 `stock` 族群 |
| 昨日漲跌／今日漲跌顯示「—」 | 舊 JSON 可能缺欄。重新執行 `TW_Strategy.py` 後，當日每筆 `metrics` 都會寫入這兩個百分比 |

---

## 7. 免責

本工具僅供研究與排程展示，不是投資建議。台股有漲跌停、除權息、調整價差，訊號可能與券商K線略有出入。
